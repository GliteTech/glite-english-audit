...
def test_a_call_is_a_session_file_not_a_batch_of_25() -> None:
    ...
    assert estimate_stage(141 * 40, 141, entry).p50_tokens == per_message + 21 * 1000
    assert (
        estimate_stage(141 * 40, 141, entry, messages_per_call=25).p50_tokens
        == per_message + 6 * 1000
    )
...
def test_estimate_stage_rejects_negative_inputs() -> None:
    ...
    with pytest.raises(ValueError, match="messages_per_call"):
        estimate_stage(100, 10, _entry(), messages_per_call=0)
