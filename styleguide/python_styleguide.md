...
conventions, typing, data handling, and the project-specific rules that keep the audit pipeline
safe and consistent.

It is adapted from the `glite-arf` Python style guide (Apache-2.0; see `NOTICE`). Where this guide
and tool configuration disagree, the configuration in `pyproject.toml` wins; report the conflict
...
4. **Centralize paths** - All runtime file locations come from `glite_english_audit.paths`;
...
serialization, CLI output). All shared enums live in `glite_english_audit.artifacts.enums`;
...
from glite_english_audit.artifacts.enums import StageStatus
...
### Centralize runtime paths in `glite_english_audit.paths`

**CRITICAL**: All runtime file locations (runtime roots per OS, run directories, snapshot
directories) come from `glite_english_audit.paths`. Never construct a home-directory or
...
run_dir = Path.home() / ".glite-english-audit" / "runs" / run_id
```
...
`glite_english_audit.diagnostics.codes`. The registry is append-only: never reuse or renumber an
...
`glite_english_audit.artifacts`; reuse them instead of defining parallel shapes.

JSON Schema files under `schemas/` are generated from the models by
`glite_english_audit.artifacts.schema_export`. Never handwrite or hand-edit a schema for a model.
...
from glite_english_audit.artifacts.io import write_model
...
For JSONL artifacts, prefer the helpers in `glite_english_audit.artifacts.io`, which validate one
...
All artifact reads and writes go through `glite_english_audit.artifacts.io`: `write_model`,
...
Never use relative imports. Always use absolute imports rooted at the `glite_english_audit`
package.
...
the project in the environment, and tests import `glite_english_audit` like any other package.
...
* `from glite_english_audit import paths` — path constructors read better as `paths.run_dir(...)`
...
```python
from glite_english_audit.artifacts import hashing
...
Every deterministic script is a module under `glite_english_audit` with a `main()` entry point,
run as `uv run python -m glite_english_audit.<module>`. No standalone scripts outside the
...
1. Take all runtime locations from `glite_english_audit.paths`; add new layout there if needed
...
4. **Take all runtime paths from `glite_english_audit.paths`**
